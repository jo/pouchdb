...
function replicate(repId, src, target, opts, returnValue, result) {
  ...
  var limit = opts.limit || false;
  var changesPending = false;     // true while src.changes is running
  var doc_ids = opts.doc_ids;
  var state = {
    cancelled: false
  };
  var FakeCheckpointer = function () {
    return {
      writeCheckpoint: function (checkpoint) {
        return utils.Promise.resolve(checkpoint);
      },
      getCheckpoint: function () {
        return utils.Promise.resolve(last_seq);
      }
    }
  };
  var CurrentCheckpointer = opts.checkpointing === false ?
    FakeCheckpointer :
    Checkpointer;
  var checkpointer = new CurrentCheckpointer(src, target, repId, state);
  var allErrors = [];
  var changedDocs = [];
  ...
  function writeDocs() {
    ...
    return target.bulkDocs({docs: docs, new_edits: false}).then(function (res) {
      ...
      if (limit && result.docs_written + result.doc_write_failures >= limit) {
        completeReplication();
