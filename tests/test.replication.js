...
interHTTPAdapters.map(function(adapters) {
  ...
  asyncTest("Test persistent replication", function() {
    var self = this;
    initTestDB(this.name, function(err, db) {
      db.bulkDocs({docs: docs}, {}, function(err, results) {
        Pouch.replicate(self.name, self.remote, {server: true, persistent: true}, function(err, result) {
          ok(result.ok, 'replication was ok');
          start();
        });
      });
  ...
  asyncTest("Test cancel persistent continuous replication", function() {
    var self = this;
    var doc1 = {_id: 'adoc', foo:'bar'};
    var doc2 = {_id: 'anotherdoc', foo:'baz'};
    initDBPair(this.name, this.remote, function(db, remote) {
      remote.bulkDocs({docs: docs}, {}, function(err, results) {
        var count = 0;
        var replicate = db.replicate.from(self.remote, {server: true, continuous: true, persistent: true});
        var changes = db.changes({
          continuous: true,
          onChange: function(change) {
            ++count;
